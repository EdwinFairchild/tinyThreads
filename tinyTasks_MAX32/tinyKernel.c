...
#define THUMB_BIT_LOCATION (1<<21)
#define BUS_FREQ           (uint64_t)SystemCoreClock// 100MHz
...
{  
    ...
    TCB_STACK[i][TOP_OF_STACK ] = THUMB_BIT_LOCATION;   // Thumb bit in PSR (Program Status Register)
    // TCB_STACK[i][TOP_OF_STACK - 2] Program Counter initialized elsewhere when we get a thread function
    TCB_STACK[i][TOP_OF_STACK - 2] = 0xDEADBEEF;           // R14 Link Register
    TCB_STACK[i][TOP_OF_STACK - 3] = 0xDEADBEEF;           // R12
    TCB_STACK[i][TOP_OF_STACK - 4] = 0xDEADBEEF;           // R3
    TCB_STACK[i][TOP_OF_STACK - 5] = 0xDEADBEEF;           // R2
    TCB_STACK[i][TOP_OF_STACK - 6] = 0xDEADBEEF;           // R1
    TCB_STACK[i][TOP_OF_STACK - 7] = 0xDEADBEEF;           // R0
    //----------------------------------------------------------------------------------------
    TCB_STACK[i][TOP_OF_STACK - 8]  = 0xDEADBEEF;           // R11   
    TCB_STACK[i][TOP_OF_STACK - 9] = 0xDEADBEEF;          // R10
    TCB_STACK[i][TOP_OF_STACK - 10] = 0xDEADBEEF;          // R9
    TCB_STACK[i][TOP_OF_STACK - 11] = 0xDEADBEEF;          // R8
    TCB_STACK[i][TOP_OF_STACK - 12] = 0xDEADBEEF;          // R7
    TCB_STACK[i][TOP_OF_STACK - 13] = 0xDEADBEEF;           // R6
    TCB_STACK[i][TOP_OF_STACK - 14] = 0xDEADBEEF;           // R5
    TCB_STACK[i][TOP_OF_STACK - 15] = 0xDEADBEEF;           // R4
    ...
    tcbs[i].sp = &TCB_STACK[i][TOP_OF_STACK - 16];
...
uint8_t tos_KernelAddThread(thread_t thread_1,thread_t thread_2,thread_t thread_3)
{
    ...
    TCB_STACK[0][TOP_OF_STACK - 2] = (uint32_t)(thread_1); 
    TCB_STACK[1][TOP_OF_STACK - 2] = (uint32_t)(thread_2);
    TCB_STACK[2][TOP_OF_STACK - 2] = (uint32_t)(thread_3);
